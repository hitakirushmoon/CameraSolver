			...
			for (int i = 0; i < 4; i++) {
				...
				if (alt < v.distance) {
					...
					while (true) {
						...
						alt += Math.abs(previous.x - previous.previous.x);
					...
					while (true) {
						...
						alt += Math.abs(previous.y - previous.previous.y);
						previous = previous.previous;
					}
...
class Node implements Comparable<Node> {
	...
	public String toString() {
		return "x: " + x + "; y: " + y + " " + Arrays.toString(costs) + distance + ", " + estimate;
	}
	...
	public int compareTo(Node o) {
		...
		if (diff == 0) {
			for (int i = 0; i < 4; i++) {
				for (int j = 0; j < 4; j++) {
					if (neighbors[i] != null) {
						diff += neighbors[i].costs[j];
					}
					if (o.neighbors[i] != null) {
						diff -= o.neighbors[i].costs[j];
					}
				}
			}
		}
