...
//			int factor = 0;
//			int x = position[ind][0];
//			int y = position[ind][1];
...
//						continue;
//					}
//					yLoop: for (int yShift = 0; yShift < h; yShift++) {
...
//							if (index == ind)
//								continue;
//							int x0 = position[index][0];
//							int y0 = position[index][1];
//
//							if ((y0 - (y - yShift)) * (y0 - (y - yShift + h)) < 0
//									&& (x0 - (x - xShift)) * (x0 - (x - xShift + w)) < 0) {
//								continue yLoop;
//							}
//
//						}
//						// raisin 0 -> count, factor 0 -> cakeSizes.size(), xShift 0 -> w, yShift 0 -> h
//						int index = yShift + h * (xShift + w * (factor + cakeSizes.size() * ind));
//
