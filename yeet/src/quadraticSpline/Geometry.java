...
class QuadraticSpline {
	...
	public QuadraticSpline(Coordinates start, Coordinates end, Coordinates startingSpeed) {
		this.start = start;
		this.end = end;
		this.start.next = end;
		c = start.x;
	...
	public Coordinates getCoordinates(double t) {
		return new Coordinates(a * t * t + b * t + c, d * t * t + e * t + f);
	}

	public Coordinates getLastSpeed() {
		return new Coordinates(2 * a + b, 2 * d + e);
	}
}

class MultiQuadraticSpline implements Renderable {
	Path curve = new Path();
	QuadraticSpline start;
	QuadraticSpline end;

	public MultiQuadraticSpline() {
	}

	public void setStart(QuadraticSpline start) {
		this.start = start;
		end = start;
	}
	...
	public void addCoordinates(Coordinates coord) {
		if (start == null) {
			start = new QuadraticSpline(coord, null, new Coordinates(0, 0));
			return;
		}
		if (start.end == null) {
			start.end = coord;
		...
		end.next = new QuadraticSpline(end.end, coord, end.getLastSpeed());
		end = end.next;
	}
	...
	public Coordinates getCoords(double t) {
		if (start == null) {
			return null;
		} else {
			Coordinates target = start.getCoordinates(t - Math.floor(t));
			curve.addCoordinates(target);
			return target;
		}
	}
	...
	@Override
	public void render(Graphics2D g) {
		Coordinates current = start.start;
		while (current != null) {
			current.render(g);
			current = current.next;
		}
